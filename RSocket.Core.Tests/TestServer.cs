...
namespace RSocket.Tests
{
	public class TestServer : RSocket
	{
		...
		{
			void IObserver<RSocketFrame>.OnCompleted() => this.Add((null, null));
			void IObserver<RSocketFrame>.OnError(Exception error) => throw new NotImplementedException(); //Next(metadata, data);
			void IObserver<RSocketFrame>.OnNext(RSocketFrame value) => this.Add((value.Metadata.ToArray(), value.Data.ToArray()));
		}
	}
}
