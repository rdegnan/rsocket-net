...
namespace RSocket
{
    ...
    public partial class RSocketProtocol
    {
        ...
        public ref struct RequestChannel
        {
            ...
            public RequestChannel(Int32 id, RSocketFrame frame, Int32 initialRequest = 0, bool follows = false, bool complete = false)
            {
                Header = new Header(Types.Request_Channel, stream: id, metadata: frame.Metadata);
                InitialRequest = initialRequest;
                DataLength = (int)frame.Data.Length;
                MetadataLength = (int)frame.Metadata.Length;
                HasFollows = follows;
            }
        ...
        public ref struct RequestStream
        {
            ...
            public RequestStream(Int32 id, RSocketFrame frame, Int32 initialRequest = 0, bool follows = false)
            {
                Header = new Header(Types.Request_Stream, stream: id, metadata: frame.Metadata);
                InitialRequest = initialRequest;
                DataLength = (int)frame.Data.Length;
                MetadataLength = (int)frame.Metadata.Length;
                HasFollows = follows;
        ...
        public ref struct RequestResponse
        {
            ...
            public RequestResponse(int id, RSocketFrame frame, int initialRequest = 0, bool follows = false)
            {
                Header = new Header(Types.Request_Response, stream: id, metadata: frame.Metadata);
                DataLength = (int)frame.Data.Length;
                MetadataLength = (int)frame.Metadata.Length;
                HasFollows = follows;
            }
            ...
            {
                var written = Header.Write(writer, Length);
                if (HasMetadata) { written += writer.WriteInt24BigEndian(MetadataLength) + writer.Write(frame.Metadata); }      //TODO Should this be UInt24? Probably, but not sure if it can actually overflow...
                written += writer.Write(frame.Data);
                return written;
            }
        ...
        public ref struct RequestFireAndForget
        {
            ...
            public RequestFireAndForget(int id, RSocketFrame frame, int initialRequest = 0, bool follows = false)
            {
                Header = new Header(Types.Request_Fire_And_Forget, stream: id, metadata: frame.Metadata);
                DataLength = (int)frame.Data.Length;
                MetadataLength = (int)frame.Metadata.Length;
                HasFollows = follows;
            }
